...
public final class Collision {
	 ...
	 * @return
	 */
	public static Obstacle[] checkCollisionProjectileToObstacle(Projectile move, float nextX, float nextY) {
		Obstacle[] sides = new Obstacle[4];
		...
		if(move.y < nextY) {
			...
			for (Obstacle obs : Game.currentMap.obstacles) {
				if(!obs.isObstacle) continue;
				if(move.x + Bullet.SIZE > obs.x && move.x < obs.x + obs.width && move.y < obs.y) {
					if(min == -1) {
						min = obs.y;
						check = obs;
					} else if(obs.y < min) {
		...
		if(move.y > nextY) {
			...
			for (Obstacle obs : Game.currentMap.obstacles) {
				if(!obs.isObstacle) continue;
				if(move.x + Bullet.SIZE > obs.x && move.x < obs.x + obs.width && move.y + Bullet.SIZE > obs.y + obs.height) {
					if(max == -1) {
						max = obs.y;
						check = obs;
					} else if(obs.y + obs.height > max) {
		...
		if(move.x < nextX) {
			...
			for (Obstacle obs : Game.currentMap.obstacles) {
				if(!obs.isObstacle) continue;
				if(move.y + Bullet.SIZE > obs.y && move.y < obs.y + obs.height && move.x < obs.x) {
					if(min == -1) {
						min = obs.x;
						check = obs;
					} else if(obs.x < min) {
		...
		if(move.x > nextX) {
			...
			for (Obstacle obs : Game.currentMap.obstacles) {
				if(!obs.isObstacle) continue;
				if(move.y + Bullet.SIZE > obs.y && move.y < obs.y + obs.height && move.x + Bullet.SIZE > obs.x + obs.width) {
					if(max == -1) {
						max = obs.x;
						check = obs;
					} else if(obs.x + obs.width > max) {
